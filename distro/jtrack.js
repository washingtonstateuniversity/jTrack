...
(function($) {
    ...
    (function(JT) {
        ...
        function _d(n){
            return (JT.defaults.debug===true) && _def(window.console) && _def(window.console.debug) && window.console.debug(n);
        }
        // @if DEBUG
        ...
        **/
        $.jtrack =    function ini(options){
            ...
            if(!$.isPlainObject(options)){
                ...
                if(_def(s.analytics)){
                    JT.defaultsettings = $.extend({}, JT.defaultsettings, s.analytics.defaults);
                    JT.accounts = s.analytics.accounts;
        ...
        JT.init_analytics = function(ga_name,callback) {
            _d('Google Analytics loaded');
            var jga = window[ga_name];
            var accounts_total = JT.accounts.length;
            $.each(JT.accounts,function(idx,acc){
                var setting,namedSpace,ns,cookiePath,cookieDomain,autoLink,sampleRate,opt,_addEvent;
                JT.settings = $.extend( {}, JT.defaultsettings, acc.settings );
                setting = JT.settings; // we are doing this to decrease the download size.  balance
                ...
                if( "object" === typeof(setting.ec) ){
                    JT.setEC(null, ga_name, ns, setting.ec);
                }
        ...
        */
        $.fn.trackPage = function(ga_name,callback) {
            ...
            if ( typeof(jga)!=='undefined' && jga.length>0) {
                ...
                if(JT.settings.onload === true || JT.settings.onload === null) {
                    $(document).ready(function(){JT.load_script(ga_name,callback);});
                } else {
                    JT.load_script(ga_name,callback);
                }
            }
        };
        ...
        */
        $.fn.trackEvent = function(category, action, label, value,ga_name,ns, callback) {
            ...
            JT.trackEvent(ele,ga_name,ns,category, action, label, value, callback);
        };

        JT.trackEvent = function(ele,ga_name,ns,category, action, label, value, callback) {
            var jga = window[ga_name];
            if(!_def(jga)) {
        ...
        *
        */
        JT.trackSocial = function(ele,ga_name,ns, network, action, target) {
            var jga = window[ga_name];
            if(!_def(jga)) {
                _d('FATAL: ga is not defined'); // blocked by whatever
            } else {
        ...
        */
        JT.setUserId = function(ele, ga_name, ns, userId) {
            var jga = window[ga_name];
            if(!_def(jga)) {
                _d('FATAL: ga is not defined'); // blocked by whatever
            } else {
        ...
        */
        JT.setEC = function(ele, ga_name, ns, ec_object) {
            var jga = window[ga_name];
            if(!_def(jga)) {
                _d('FATAL: ga is not defined'); // blocked by whatever
            } else {
        ...
        *
        */
        JT.trackPageview = function(pageTracker,uri) {
            if(!_def(pageTracker)) {
        ...
        *
        */
        JT.hit = function(ele, ga_name, ns, hitType, hitPage) {
            var jga = window[ga_name];
            if(!_def(jga)) {
        ...
        */
        $.fn.jtrack = function(options,ga_name,ns) {
            ...
            return $.each($(this),function() {
                ...
                ele            = $(this);
                settings    = $.extend({}, JT.eventdefaults, options);
                ...
                if (ele.is('.tracked') && overwrites === 'false') {
                    ...
                    ele.on(tasactedEvent, function(e) {
                        ...
                        if(false !== JT.settings.userId){
                            JT.setUserId(ele, ga_name, ns, JT.settings.userId);
                        }

                        if( false !== settings.ec && ! $.isEmptyObject(settings.ec) ){
                            JT.setEC(ele, ga_name, ns, settings.ec);
                        }
                        ...
                        if(mode.indexOf("_social")>-1 ){
                            ...
                            JT.trackSocial(ele,ga_name,ns,network,socialAction);
                        }
